...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM funeral_applications WHERE id = $record_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send'])) {
    $update_query = "UPDATE funeral_applications SET is_forwarded = 1 WHERE id = $record_id";
    
    if ($conn->query($update_query) === TRUE) {
        header("Location: adminFuneralTable.php"); // Redirect after successful update
        exit;
    } else {
...
<head>
    ...
    <script>
        function btnback(event) {
            ...
            window.location.href = "adminFuneralTable.php";
        }
    </script>
</head>
<body>
    ...
    <div class='container-fluid'>
        <div class='row'>
            ...
            <div class="container mt-5">
                <h2 id="baptismApplicationLabel">Funeral Application Details</h2>
                <div id="userReqInfo">
                    <div id="forLabel">
                    ...
                    <div id="applicantInfos" class="d-flex">
                        <div id="leftSide">
                            <p class="applicantData"><?php echo htmlspecialchars($record['deceased_name']); ?></p>
                            <p class="applicantDataLabel">Name of Deceased</p>
                        ...
                        <div id="rightSide">
                            <p class="applicantData"><?php echo htmlspecialchars($record['requestor_name']); ?></p>
                            <p class="applicantDataLabel">Name of Requestor</p>
                            <p class="applicantData"><?php echo htmlspecialchars($record['time_of_mass']); ?></p>
                            <p class="applicantDataLabel">Time of Mass</p>
                            <p class="applicantData"><?php echo htmlspecialchars($record['service_place']); ?></p>
                            <p class="applicantDataLabel">Place of Service</p>
                        </div>
                    </div>
                    <div id="forComments">
                        <div id="bottomSide">
